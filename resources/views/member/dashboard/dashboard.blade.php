					...
					<table class="table text-nowrap" id="dashboardCompetition">
						...
						<tbody>
							...
							<tr class="@if($value->win_status==1) bg-gold @endif">
								<td>{{$no++}}</td>
								<td>{{$value->competition_join->payment->payment_number}}</td>
								<td>{{$value->competition_join->competition->title}}</td>
								<td>{{$value->categories->name}}</td>
								<td>{{\Carbon\Carbon::parse($value->competition_join->join_date)->setTimezone('Asia/Jakarta') }}</td>
								<td>
									@if($value->categories->free==0)
									@if($value->submision_status==0)
									<span class="badge badge-warning">
										Belum Diajukan
									</span>
									@endif
									@if($value->submision_status==1)
								...
								<td>
									...
									<div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
										<a href="{{url('competition/detail')}}/{{$value->id}}" class="btn btn-secondary text-white"><i class="fa-regular fa-eye"></i></a>
										@if($value->win_status==1)
										<a target="_blank" href="{{url('certificate/')}}/{{$value->certificate->id}}" class="btn btn-secondary text-white"><i class="fa-solid fa-medal"></i></a>
										@else
